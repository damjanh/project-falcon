...
public class HeightMapMesh {
    ...
    public static final float STARTX = -15.f;
    public static final float STARTZ = -15.f;
    private static final int MAX_COLOUR = 255 * 225 * 225;
    ...
    public HeightMapMesh(float minY, float maxY, float[][] heightMap, String textureFile) throws Exception {
        this.minY = minY;
        this.maxY = maxY;
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                float currentHeight = heightMap[row][col] * 15;
                heightArray[row][col] = currentHeight;
                positions.add(currentHeight); //y
                positions.add(STARTZ + row * incz); //z

                // Set texture coordinates
                textCoords.add((float) col / (float) width);
                textCoords.add((float) row / (float) height);
